#!/usr/bin/env node
// [Input] Admin Drizzle history and the isolated Registry111 PostgreSQL integration test.
...
// [Sync] 2026-09-15: verify Story Workspace review DTO/ORM transactions on PostgreSQL.
...
if (process.argv.length !== 2) throw new Error("Usage: node scripts/run-story-workspace-review-contract.mjs");
const root = process.cwd();
const temporaryRoot = await mkdtemp(join(tmpdir(), "ink-story-workspace-review-contract-"));
const suffix = randomBytes(5).toString("hex");
const databaseName = `ink_story_workspace_review_test_${suffix}`;
...
try {
  ...
  const migrationEnvironment = {
    ...
    AI_CREDENTIAL_ENCRYPTION_KEY_ID: `story-review-test-${suffix}`,
  ...
  try {
    ...
    await admin.query(`CREATE ROLE ink_story_review_executor LOGIN PASSWORD '${executorPassword}'`);
    await admin.query(`GRANT CONNECT ON DATABASE ${databaseName} TO ink_story_review_executor`);
    await admin.query("GRANT USAGE ON SCHEMA public, dream TO ink_story_review_executor");
    await admin.query(`GRANT SELECT, UPDATE ON
      story_workspace_stories, story_workspace_characters, story_workspace_scenes
      TO ink_story_review_executor`);
    await admin.query(`GRANT SELECT ON story_workspace_story_characters TO ink_story_review_executor`);
    await admin.query(`GRANT SELECT, INSERT ON dream.operation_receipts TO ink_story_review_executor`);
    await admin.query(`GRANT INSERT ON admin_audit_logs TO ink_story_review_executor`);
  } finally {
    await admin.end();
  }
  const restricted = new URL(adminUrl);
  restricted.username = "ink_story_review_executor";
  restricted.password = executorPassword;
  await run("pnpm", ["exec", "vitest", "run", "app/lib/dream/storyWorkspaceReviewPostgres.integration.test.ts"], {
    ...process.env,
    STORY_WORKSPACE_REVIEW_TEST_ADMIN_URL: adminUrl,
    STORY_WORKSPACE_REVIEW_TEST_DATABASE_URL: restricted.toString(),
  });
  receipt = { status: "passed", database: databaseName,
    executor: "ink_story_review_executor", migration_source: "Admin Drizzle" };
} finally {
