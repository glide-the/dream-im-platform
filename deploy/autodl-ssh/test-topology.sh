...
# [Sync] 2026-08-26: cover separated PostgreSQL home/data topology.
set -euo pipefail
...
grep -Fx "EMBEDDED_POSTGRES_DATA_DIR=${ADMIN_HOME}/data/postgres" "${OUTPUT_ENV}"
...
[[ "$(mode_of "${ADMIN_HOME}/data/postgres")" == "700" ]]
[[ "$(owner_of "${ADMIN_HOME}/data/postgres")" == "${CURRENT_USER}:${CURRENT_GROUP}" ]]
[[ "$(mode_of "${DATA_ROOT}/artifacts")" == "750" ]]

rm -rf "${ADMIN_HOME}/data/postgres"
...
mkdir -p "${ADMIN_HOME}/data/postgres"
