          ...
          <div className="admin-dialog-frame">
            ...
            <div className="admin-dialog-body">
              <form id="billing-credit-form" className="space-y-5" onSubmit={submit}>
                <label className="block text-xs font-semibold text-text-secondary">搜索计费用户<input className="admin-field mt-2 text-sm" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Email、显示名或源用户 ID" /></label>
                <label className="block text-xs font-semibold text-text-secondary">计费用户 *<select className="admin-field mt-2 font-mono text-xs" value={platformUserId} onChange={(event) => setPlatformUserId(event.target.value)} required disabled={users.query.isLoading}><option value="">{users.query.isLoading ? "正在加载…" : "选择真实计费用户"}</option>{matchingUsers.map((user) => <option key={String(user.id)} value={String(user.id)}>{String(user.email ?? user.external_user_id ?? user.id)} · {String(user.tier ?? "—")}</option>)}</select></label>
