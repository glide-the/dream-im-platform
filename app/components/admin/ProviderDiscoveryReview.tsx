...
export default function ProviderDiscoveryReview({ providerId, snapshotId }: { providerId: string; snapshotId: string }) {
  ...
  const selectableVisible = visible.filter((item) => item.state !== "conflict");
  ...
  return (
    <section className="fixed inset-0 z-[80] flex min-h-[100dvh] flex-col overflow-hidden bg-bg-primary">
      <header className="shrink-0 border-b border-border bg-bg-surface">
        <div className="mx-auto flex min-h-20 max-w-[1360px] items-center justify-between gap-4 px-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-4"><Link href="/admin/models/providers" className="grid h-11 w-11 place-items-center rounded-xl border border-border" aria-label="返回 Provider">←</Link><div className="min-w-0"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-text-tertiary">cc-switch · model fetch · diff review</p><h1 className="truncate font-display text-2xl font-semibold">模型目录差异确认</h1></div></div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <div className="mx-auto grid max-w-[1360px] gap-5 px-4 py-6 sm:px-6 lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
            ...
            <dl className="grid grid-cols-3 rounded-2xl border border-border bg-bg-surface text-center"><div className="p-4"><dt className="text-[10px] text-text-tertiary">新增</dt><dd className="mt-1 font-mono text-lg font-semibold text-accent">{counts.new}</dd></div><div className="border-x border-border p-4"><dt className="text-[10px] text-text-tertiary">已有</dt><dd className="mt-1 font-mono text-lg font-semibold">{counts.existing}</dd></div><div className="p-4"><dt className="text-[10px] text-text-tertiary">冲突</dt><dd className="mt-1 font-mono text-lg font-semibold text-danger">{counts.conflict}</dd></div></dl>
            <p className="rounded-2xl border border-border bg-bg-secondary p-4 text-xs leading-5 text-text-secondary">新增模型以 disabled 写入；已有模型只刷新目录元数据；冲突项禁止勾选。模型启用和价格配置仍是后续独立操作。</p>
          </aside>
          <main className="min-w-0 overflow-hidden rounded-2xl border border-border bg-bg-surface">
            <div className="grid gap-3 border-b border-border bg-bg-secondary/35 p-4 sm:grid-cols-[minmax(200px,1fr)_auto]"><input className="admin-field min-h-11 rounded-xl bg-bg-surface text-sm" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索模型 ID、Owner 或 alias" aria-label="搜索模型差异" /><div className="flex max-w-full gap-1 overflow-x-auto rounded-xl bg-bg-secondary p-1">{(["all", "new", "existing", "conflict"] as const).map((state) => <button key={state} type="button" onClick={() => setFilter(state)} className={`min-h-9 whitespace-nowrap rounded-lg px-3 text-xs font-semibold ${filter === state ? "bg-bg-surface shadow-soft" : "text-text-tertiary"}`}>{state === "all" ? "全部" : state === "new" ? "新增" : state === "existing" ? "已有" : "冲突"}</button>)}</div></div>
            <div className="max-w-full overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead><tr className="border-b border-border">{["选择", "上游模型 ID", "Owner", "拟用 alias", "差异状态"].map((label) => <th key={label} className="px-4 py-3 font-mono text-[10px] uppercase text-text-tertiary">{label}</th>)}</tr></thead><tbody>{visible.map((item) => <tr key={item.id} className="border-b border-border align-top"><td className="px-4 py-4"><input type="checkbox" className="h-5 w-5 accent-current" checked={selected.has(item.id)} disabled={item.state === "conflict" || unavailable} onChange={() => toggle(item.id)} aria-label={`选择 ${item.id}`} /></td><td className="px-4 py-4"><span className="font-mono text-xs font-semibold">{item.id}</span><span className="mt-1 block text-xs text-text-tertiary">{item.displayName}</span></td><td className="px-4 py-4 text-xs text-text-secondary">{item.ownedBy ?? "—"}</td><td className="px-4 py-4 font-mono text-xs">{item.proposedCode}</td><td className="px-4 py-4"><span className={`rounded-full border px-2 py-1 text-[10px] font-semibold ${item.state === "conflict" ? "border-danger/35 bg-danger-light text-danger" : item.state === "new" ? "border-accent/35 bg-accent-light text-accent" : "border-border bg-bg-secondary text-text-secondary"}`}>{item.state}</span>{item.conflictReason ? <p className="mt-2 max-w-xs text-xs leading-5 text-danger">{item.conflictReason}</p> : null}</td></tr>)}{!snapshot ? <tr><td colSpan={5} className="p-12 text-center text-sm text-text-tertiary">正在读取差异快照…</td></tr> : visible.length === 0 ? <tr><td colSpan={5} className="p-12 text-center text-sm text-text-tertiary">没有匹配的差异项</td></tr> : null}</tbody></table></div>
