...
export default function PricingSyncReview({ snapshotId }: { snapshotId: string }) {
  ...
  return (
    <section className="fixed inset-0 z-[80] flex min-h-[100dvh] flex-col overflow-hidden bg-bg-primary">
      <header className="shrink-0 border-b border-border bg-bg-surface"><div className="mx-auto flex min-h-20 max-w-[1440px] items-center justify-between gap-4 px-4 sm:px-6"><div className="flex min-w-0 items-center gap-4"><Link href="/admin/models/pricing" className="grid h-11 w-11 place-items-center rounded-xl border border-border" aria-label="返回 Pricing">←</Link><div><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-text-tertiary">cc-switch · models.dev · versioned apply</p><h1 className="font-display text-2xl font-semibold">价格目录差异确认</h1></div></div><span className="hidden font-mono text-[10px] text-text-tertiary sm:block">CATALOG · {snapshot?.catalog_version ?? "loading"}</span></div></header>
      <div className="min-h-0 flex-1 overflow-y-auto"><div className="mx-auto grid max-w-[1440px] gap-5 px-4 py-6 sm:px-6 lg:grid-cols-[290px_minmax(0,1fr)]">
        ...
        <main className="min-w-0 overflow-hidden rounded-2xl border border-border bg-bg-surface"><div className="grid gap-3 border-b border-border bg-bg-secondary/35 p-4 sm:grid-cols-[minmax(200px,1fr)_auto]"><input className="admin-field min-h-11 rounded-xl bg-bg-surface text-sm" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索本地 alias、上游模型或 Provider" aria-label="搜索价格匹配" /><div className="flex max-w-full gap-1 overflow-x-auto rounded-xl bg-bg-secondary p-1">{(["all", "exact", "normalized", "ambiguous", "unmatched"] as const).map((state) => <button key={state} type="button" onClick={() => setFilter(state)} className={`min-h-9 whitespace-nowrap rounded-lg px-3 text-xs font-semibold ${filter === state ? "bg-bg-surface shadow-soft" : "text-text-tertiary"}`}>{state === "all" ? "全部" : state}</button>)}</div></div>
