...
export function StoryResourceView({ kind }: StoryResourceViewProps) {
  ...
  if (kind === "stories") return <AdminResourceManager resource="story-stories" title="剧本" description="直接读取 Dream canonical Story；正文不进入列表或详情 payload，运营仅维护批准字段与审核命令。" canCreate={false} canDelete={false} container="drawer" fields={[{ key: "title", label: "标题", control: "text", section: "main", required: true }, { key: "description", label: "说明", control: "textarea", section: "main", nullable: true }, { key: "type", label: "类型", control: "select", section: "main", options: storyTypeOptions }]} commands={[{ action: "confirm", label: "确认", description: "仅确认当前剧本的审核状态，不改写正文或其他业务实体。", tone: "success" }]} defaultSort="updated_at" filters={[{ field: "title", label: "剧本标题" }, { field: "workspace_id", label: "所属工作区", operator: "eq", control: "relation", relation: { resource: "story-workspaces", labelKey: "name", secondaryKey: "id", searchField: "name" } }, { field: "author_id", label: "作者", operator: "eq", control: "relation", relation: { resource: "users", labelKey: "email", secondaryKey: "id", searchField: "email" } }, { field: "type", label: "类型", operator: "eq", control: "select", options: storyTypeOptions }, { field: "review_status", label: "审核状态", operator: "eq", control: "select", options: reviewStatusOptions }, { field: "status", label: "业务状态", operator: "eq", control: "select", options: storyStatusOptions }, { field: "updated_from", apiField: "updated_at", label: "更新起始", operator: "gte", control: "datetime" }, { field: "updated_to", apiField: "updated_at", label: "更新截止", operator: "lte", control: "datetime" }]} columns={[{ key: "title", label: "剧本标题" }, { key: "id", label: "Story ID", format: "copy" }, { key: "workspace_name", label: "所属工作区" }, { key: "author_label", label: "作者" }, { key: "billing_identity_bound", label: "计费映射", format: "binding" }, { key: "type", label: "类型", format: "status" }, { key: "review_status", label: "审核状态", format: "status" }, { key: "status", label: "业务状态", format: "status" }, { key: "updated_at", label: "更新时间", format: "date" }]} />;
