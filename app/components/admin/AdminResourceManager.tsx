...
export type AdminResourceManagerProps = {
  ...
  renderDetail?: (record: Record<string, unknown>) => ReactNode;
};
...
function statusClass(value: unknown) {
  ...
  if (["active", "enabled", "confirmed", "completed", "published", "success", "settled", "indexed"].includes(status)) return "border-success/35 bg-success-light text-success";
  if (["failed", "missing", "rejected", "disabled", "closed", "cancelled", "settlement_failed", "revoked"].includes(status)) return "border-danger/35 bg-danger-light text-danger";
...
function commandIsVisible(
  ...
  if (command.visible) return command.visible(record);
  if (resource === "story-stories" && command.action === "confirm") {
    return (
      Boolean(record.script_revision) &&
      record.review_status === "pending" &&
      [1, "1", true].includes(record.agent_generated as never)
    );
  }
  return true;
}
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (mode === "create" && result.data?.plaintextKey) {
        ...
        setOneTimeGatewayBaseUrl(String(result.data.gatewayBaseUrl ?? ""));
        setMode("detail");
      } else {
        close(true);
      }
