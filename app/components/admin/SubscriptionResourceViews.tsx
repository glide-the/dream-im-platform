...
export function SubscriptionPlansView() {
  const fields: AdminFieldDefinition[] = [
    ...
    { key: "displayEyebrow", sourceKey: "display_eyebrow", label: "Eyebrow", control: "text", section: "display", nullable: true, help: "Dream套餐卡片名称上方的短叙事标签。" },
    { key: "displayNote", sourceKey: "display_note", label: "Note", control: "textarea", section: "display", nullable: true },
    { key: "displayDetails", sourceKey: "display_details", label: "Details", control: "json", section: "display", required: true, help: "严格JSON字符串数组；由Dream Product API直接展示。" },
    { key: "status", label: "状态", control: "select", section: "lifecycle", updateOnly: true, options: planStatus },
  ];
  return <AdminResourceManager resource="subscription-plans" title="订阅套餐" description="套餐只保存稳定身份与Dream正式展示文案；不可覆盖的 Plan Version 定义每位用户个人订阅周期中的月度 Token 额度与模型权益。" canDelete={false} container="drawer" createLabel="新建套餐" sections={[{ id: "identity", title: "套餐身份" }, { id: "display", title: "Dream展示层级", description: "eyebrow → name → note → details；前端不得复制静态套餐文案。" }, { id: "lifecycle", title: "生命周期", description: "退役只阻止后续选择，已有订阅继续保留其历史版本快照。" }]} fields={fields} createDefaults={{ displayDetails: [] }} filters={[{ field: "name", label: "名称或 Code" }, { field: "status", label: "状态", operator: "eq", options: planStatus }]} columns={[{ key: "code", label: "Code" }, { key: "name", label: "名称" }, { key: "display_eyebrow", label: "Eyebrow" }, { key: "status", label: "状态", format: "status" }, { key: "version_count", label: "版本数" }, { key: "updated_at", label: "更新时间", format: "date" }]} />;
}
...
export function SubscriptionEntitlementsView() {
  const fields: AdminFieldDefinition[] = [
    ...
    { key: "isDefault", sourceKey: "is_default", label: "默认模型", control: "switch", section: "access", help: "每个Plan Version最多一个；Claude Agent只在没有有效保存值时使用Free版本的默认alias。" },
    { key: "enabled", label: "启用权益", control: "switch", section: "access" },
  ];
  return <AdminResourceManager resource="subscription-entitlements" title="模型权益" description="每个版本按模型定义 Gateway Scope、默认alias、独立安全阈值与 Storage 上限；发布版本的权益不可修改。" canDelete={false} container="drawer" createLabel="添加模型权益" sections={[{ id: "relation", title: "版本与模型" }, { id: "access", title: "调用范围" }, { id: "limits", title: "独立安全上限", description: "日/自然月上限用于风险控制，不发放额度，也不替代用户个人订阅周期中的 Token Allowance。" }]} fields={fields} createDefaults={{ gatewayScopes: ["messages:create", "chat:create", "models:list"], dailyTokenLimit: null, monthlyTokenLimit: null, storageBytesLimit: null, isDefault: false, enabled: true }} filters={[{ field: "plan_code", label: "套餐" }, { field: "model_code", label: "模型" }, { field: "enabled", label: "启用", operator: "eq", options: [{ label: "启用", value: "true" }, { label: "停用", value: "false" }] }]} columns={[{ key: "plan_code", label: "套餐" }, { key: "version_number", label: "版本" }, { key: "model_code", label: "模型" }, { key: "gateway_scopes", label: "Scopes", format: "json" }, { key: "is_default", label: "默认", format: "boolean" }, { key: "daily_token_limit", label: "日 Token 安全上限" }, { key: "monthly_token_limit", label: "自然月 Token 安全上限" }, { key: "storage_bytes_limit", label: "Storage" }, { key: "enabled", label: "状态", format: "boolean" }]} />;
}
