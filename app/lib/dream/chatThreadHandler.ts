...
// [Sync] 2026-09-15: bind OAuth/runtime actors to exact Thread operations.
import { z } from "zod";
...
export async function handleChatThreadOperation(request: Request, operationName: string) {
  return handleInternalAuthRequest(request, async (service, setRequestId) => {
    ...
    const bearer = request.headers.get("authorization") ?? "", isDelegated = bearer.startsWith("Bearer idg_");
    const requirements = [identitySchemaRequirement, ...chatThreadSchemaRequirements, ...(isDelegated ? [runtimeDelegationSchemaRequirement, runtimePurposeSchemaRequirement] : [])];
    return withDataTransaction(requirements, async tx => {
      ...
      const actor = await requireDataActor(tx, request.headers, service, operation.kind === "read" ? "dream:read" : "dream:write", threadId);
