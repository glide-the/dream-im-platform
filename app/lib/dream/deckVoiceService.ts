...
export function configuredDeckVoicePolicy(){let raw:unknown;try{raw=JSON.parse(requiredAuthValue("DREAM_DECK_POLICY_JSON"));}catch{throw new AuthBoundaryError("DECK_POLICY_NOT_CONFIGURED");}const parsed=dto.deckVoicePolicyDto.safeParse(raw);if(!parsed.success)throw new AuthBoundaryError("DECK_POLICY_NOT_CONFIGURED");return parsed.data;}
