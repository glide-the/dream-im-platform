// [Input] Frozen Registry174 prefix, Registry175-182 DTOs, generated inventory and production POST route.
// [Output] Append-only hashes, schema requirements and dedicated dispatch evidence.
// [Pos] Registration gate for the shared Claude Plugin persistence aggregate.
// [Sync] 2026-09-16: append Registry175-182 without changing Registry174 bytes.
...
import generated182 from "../../../docs/architecture/admin-dream-operation-contracts.json";
...
const names = ["claude-plugin.installations.list", "claude-plugin.marketplace.list",
  ...
  "claude-plugin.installation.read", "claude-plugin.install.report", "claude-plugin.installation.uninstall"] as const;
...
it("preserves Registry174 and appends exactly Registry175-182", () => {
  expect(dreamOperations).toHaveLength(182); expect(generated182).toEqual(dreamOperations);
...
it("dispatches Registry175-182 through the dedicated handler", async () => {
