...
// [Output] Exact Registry121 hash, audience, scope and fixed-name dispatch assertions.
// [Pos] Registration gate for claim-fenced confirmation Runtime persistence.
// [Sync] 2026-09-16: append claim-turn without changing Registry120 bytes.
...
import generated121 from "../../../docs/architecture/admin-dream-operation-contracts.json";
...
it("preserves Registry120 and appends exactly one Registry121 operation", () => {
  expect(dreamOperations).toHaveLength(121);
  expect(generated121).toEqual(dreamOperations);
  expect(createHash("sha256").update(canonicalContractJson(dreamOperations.slice(0, 120))).digest("hex"))
