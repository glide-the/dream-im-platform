...
// [Sync] 2026-09-15: append catalog Workspace/read/patch DTO/ORM operations without changing Registry111 bytes.
...
it("preserves Registry111 and appends exactly three Registry114 operations", () => {
  ...
  expect(dreamOperations.slice(111).map(item => item.capability.contract_sha256)).toEqual([
  ...
  for (const registered of dreamOperations.slice(111)) expect(registered.requirements)
