...
// [Sync] 2026-09-15: reject entity and Reflections worker bearers at OAuth-only service boundaries.
...
export async function principalForServiceToken(database: Parameters<typeof principalForAccessToken>[0], token: string, service: DreamServiceClient, scope?: string) {
  ...
  if (token.startsWith("rta_")) throw new AuthBoundaryError("REFLECTION_AUTHORITY_OPERATION_DENIED", 403);
