...
vi.mock("@aws-sdk/s3-request-presigner", () => ({
  getSignedUrl: vi.fn(
    async (_c: unknown, _cmd: unknown, { expiresIn }: { expiresIn: number }) =>
...
vi.mock("@aws-sdk/client-s3", () => {
  ...
  // Create a proper mock class for S3Client
  class MockS3Client {
    send = sendMock;
    constructor(_config: Record<string, unknown>) {
      // Config is received but not used in mock
    }
  }
...
describe("s3-file-storage", () => {
  ...
  it("exists returns true/false via HeadObject", async () => {
    ...
    // false
    const err: Error & { $metadata?: { httpStatusCode?: number } } = new Error("not found");
