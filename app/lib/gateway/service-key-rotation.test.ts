...
// [Sync] 2026-09-16: cover exact Dream Runtime scope rotation without SQL or plaintext receipts.
...
function databaseFor(current: ReturnType<typeof createGatewayApiKey>) {
        ...
        where: vi.fn(() => ({
          limit: vi.fn(() => {
            const rows = [{
              ...
              keyHash: current.hash, scopes: ["messages:create", "models:list"], expiresAt: null,
            }];
