...
import { GatewayUserDefaultLimitsView, ModelPermissionsResourceView } from "@/components/admin/AdminResourceViews";

export default function GatewayRateLimitsPage() {
  return <AdminModulePage eyebrow="Proxy gateway / rate limits" title="限流策略" description="在可编辑策略区调整用户、模型和套餐上限；实时窗口只记录已经发生的请求与 Token 用量。" status="策略可编辑 · 计数只读" tabs={gatewayTabs}>
    <section className="admin-panel p-5" aria-labelledby="rate-limit-data-model-title">
    ...
    <AdminResourceTable resource="gateway-rate-limits" title="实时用量计数（只读）" description="Gateway 按用户、模型和分钟／日／月窗口自动累加请求与 Token。该区域不是配置策略，不提供编辑或清零；修改上限请使用上方策略区。" defaultSort="window_start" filters={[{ field: "email", label: "用户邮箱" }, { field: "model_code", label: "模型" }, { field: "window_type", label: "窗口", operator: "eq" }]} columns={[{ key: "email", label: "用户" }, { key: "model_code", label: "模型" }, { key: "window_type", label: "窗口", format: "status" }, { key: "window_start", label: "窗口开始", format: "date" }, { key: "request_count", label: "请求数" }, { key: "token_count", label: "Token" }, { key: "updated_at", label: "更新时间", format: "date" }]} />
  </AdminModulePage>;
}
