...
<!-- [Sync] 2026-09-17: record Dream client-local logout/SSO re-entry while keeping natural expiry and model gates separate. -->
<!-- [Sync] 2026-09-16: align normal activation with 0063, 64 migrations, nine capabilities and completed Google/Device acceptance. -->
...
AUTH负责identity协议/BFF、Dream subject links、active canonical/platform读取，并EXECUTE `identity.register_canonical_user(text,text,text)`；它不能直接插入users、订阅或账本。Admin管理Session使用独立、最小授权的Admin认证UOW读写`admin_users/admin_sessions`、读取RBAC并追加login audit，不读取Dream user或subject link。注册函数由前向0056发布，fixed pg_catalog search_path/qualified tables/明确旧关系与Free/default-model readiness在同一事务，PUBLIC EXECUTE撤销。CONTROL当前明确负责Admin bootstrap/RBAC与identity provision；Dream Better Auth Session不得以邮箱、Google或同email自动提升Admin。
