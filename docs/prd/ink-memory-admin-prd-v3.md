...
| 分层 | 本文定义 |
|---|---|
| **Current** | Admin 已有 Plan/Version/Entitlement/Subscription/Allowance、Gateway、Usage/Ledger 基线，但 Subscription 错误混入 currency、price、金额额度、cash overage 和平台 `effective_from`；另有 canonical 用户反查、前 50/100 条 selector、cash-only 放行、402 单位与终态不可变缺口。Dream 仍运行 SQLite、静态订阅页和直连 Provider 链路。 |
| **Target** | 单一 PostgreSQL `ink-memory`、唯一 canonical `users`、Token-only 用户级月度订阅、不可覆盖套餐/权益、Dream 产品 API，以及严格 Gateway 资格与 Token 结算。Provider Pricing、现金账户和金额 Ledger 保持独立域。 |
| **Release Gate** | 43+5 迁移演练、Token-only 合同/月度锚点、产品 API/页面、Gateway canary、Token/财务各自守恒、Secret 治理、隔离 PostgreSQL 及 1440×1000/390×844 自动化验证全部通过；未过不得把 Target 写成 Implemented。 |
| **Deferred** | 订阅支付、PaymentAdapter/Webhook、真实 Stripe/支付宝/微信支付/银行渠道，以及尚无 streaming-audio capability/计量合同的 ASR Gateway 接入。Deferred 能力不进入 Subscription 依赖、导航或 release gate，不模拟成功。 |
...
flowchart LR
  ...
  U -. "Current legacy only: cash-only mode" .-> B
```
...
5. Subscription Plan/Version 只保存 Token 与非货币权益：无 currency、price、金额额度、cash overage、Payment 或平台统一生效日；每个用户使用自己的月度 `[start,end)`。Provider Pricing 和独立账务金额仍为整数 micro-USD。Plan Version、Entitlement 和 Pricing 版本不可覆盖；Usage、Ledger、Subscription Event 与 Audit 只追加或终态不可变。
...
8. 订阅支付与所有 PaymentAdapter/Webhook 能力延期且与 Token-only Subscription 解耦；未单独立项前不创建支付页面、可点入口、测试成功态或订阅金额字段。
...
| 订阅与权益 | [03-subscriptions](modules/03-subscriptions.md) | `/admin/subscriptions/**` | Current 错误混入金额/全局生效日；Target 是 Token-only 用户月度周期、下一周期换版、自动推进与 Dream 产品 API |
...
| Usage 与独立账务 | [06-billing](modules/06-billing.md) | `/admin/billing/**` | Current 有 Usage/账户/Ledger/报表/credit；这些金额事实不属于套餐权益。Target 补终态不可变；支付协作 Deferred |
...
| 支付协作 | Deferred；尚无权威表或运行时服务 | 未实现 | 不属于本期 Subscription/Gateway 依赖 |
...
| P0（workspace 已纠偏，发布待验证） | 历史 Schema/数据仍物理保留套餐 currency、基础价、金额额度、cash overage、annual 与 `effective_from`；当前 workspace 的 `0017`、API、Gateway 和 UI 已停止新货币语义，但尚未在显式隔离 PostgreSQL 与双视口 E2E 中验证 | 按 [Token-only 订阅 PRD](modules/03-subscriptions.md) 完成隔离迁移/事务/视觉门禁后发布；历史列/Ledger 只读保留，不破坏删除 |
| P1 | 生命周期没有周期推进任务；续期可提前建立未来周期，月末可能漂移；升级立即重开周期并发完整额度，`cancel_at_period_end` 缺少无副作用撤销 | 补锚点月度推进、边界幂等 worker、下一周期升降级和真正撤销取消；任何动作不得产生订阅金额或重复 Token |
...
| P0 | Dream 产品 API 与真实 Token-only 订阅页不存在 | 完成服务间身份、Token-only 投影、周期/幂等命令和错误合同；页面不留静态价格、假额度或支付 fallback。PaymentAdapter/Webhook 维持 Deferred，不作为本期阻断依赖 |
...
- Dream 43+5 PostgreSQL、真实 Token-only 订阅体验与 Gateway 接入进入 Target 实施范围；计费、订阅支付和新推理服务仍按独立阶段处理，遵循 [`ink-dream-memory/README.md`](../architecture/ink-dream-memory/README.md)。本文不授权把逻辑 owner 当作 `ALTER OWNER`/GRANT/REVOKE 授权；迁移前必须只读盘点真实 owner/ACL/约束/已有数据。
...
- Subscription 主路径按 `User → Subscription → Plan Version → Entitlement → Model Permission → Current-period Token Allowance → Request → Token Usage` 执行并冻结快照；Provider Pricing/现金账户/Ledger 是独立链路。发布前 cash-only 默认放行必须经 canary 退出，不能变成套餐 overage 或长期分支。
...
- 重复订阅命令、并发升降级、月末续期和 Token reserve/capture/release 均有竞态/属性测试；本阶段没有 PaymentAdapter/Webhook 或真实支付网络请求。
